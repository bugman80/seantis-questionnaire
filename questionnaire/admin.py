...
class QuestionAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        """Hack to have Questionnaire list accessible for custom
        changelist template
        """
        ...
        questionnaire_id = request.GET.get(
            'questionset__questionnaire__id__exact',
            None
        )
        if questionnaire_id:
        ...
        extra_context['questionnaires'] = Questionnaire.objects.filter(
            **args).order_by('name')
        return super(QuestionAdmin, self).changelist_view(request,
                                                          extra_context)
...
class RunInfoAdmin(admin.ModelAdmin):
    list_display = ['random', 'runid', 'subject', 'created', 'emailsent',
                    'lastemailerror']
    pass
